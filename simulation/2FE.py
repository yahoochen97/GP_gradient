...
class GPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, ard_num_dim=None):
        ...
        self.x_mean_module = MaskMean(active_dims=[0,1], \
                base_mean=LinearMean(input_size=2, bias=False))
        self.i_mean_module = MaskMean(active_dims=2, \
                base_mean=ConstantVectorMean(d=train_x[:,2].unique().size()[0]))
        self.t_mean_module = MaskMean(active_dims=3, \
                base_mean=ConstantVectorMean(d=train_x[:,3].unique().size()[0]))
        self.covar_module = ScaleKernel(RBFKernel(active_dims=[0,1], ard_num_dims=2))

    def forward(self, x):
        mean_x = self.x_mean_module(x) + self.i_mean_module(x) + self.t_mean_module(x)
...
def main(args):
    ...
    result_filename = "./results/2FE_N" + str(N) + "_T" + str(T) + "_SEED" + str(SEED) + ".csv"
